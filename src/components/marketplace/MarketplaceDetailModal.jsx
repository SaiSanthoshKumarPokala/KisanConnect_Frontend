        ...
        >
          {activeTab === "about" && (
            <div className="border-b border-gold/20 p-5 lg:border-b-0 lg:border-r">
              ...
              <div className="rounded-2xl border border-gold/20 bg-darkgreen p-4">
                <h4 className="text-base font-bold text-gold">About This Listing</h4>
          ...
          >
            {activeTab === "about" && (
              <>
                <div>
                  <h3 className="text-2xl font-black leading-tight text-gold md:text-3xl">
                    {item.name}
                  </h3>
                  <p className="mt-1 text-sm text-white/80">
                    Listed by farmer · {item.location}
                  </p>
                </div>

                <div className="rounded-2xl border border-gold/20 bg-darkgreen p-4">
                  {[
                    { label: "Price", value: `₹${item.price} per kg` },
                    { label: "Category", value: item.category },
                    { label: "Stock", value: formatStock(item.stock) },
            ...
            {activeTab === "book" && (
              <div className="w-full max-w-2xl rounded-3xl border border-gold/25 bg-linear-to-b from-[#101f15] to-[#070d09] p-6 shadow-2xl">
                <div>
                  <h3 className="text-lg font-black text-amber-200">Marketplace Purchase</h3>
                ...
                >
                  <div className="grid gap-3 sm:grid-cols-2">
                    <label className="flex flex-col gap-1.5">
                      <span className="text-xs font-bold text-amber-200">Quantity (kg) *</span>
                      <input
                        type="number"
                        min="1"
                        max={item.stock}
                        required
                        placeholder={`Max ${item.stock} kg`}
                        value={bookingForm.quantity}
                        onChange={(e) =>
                          setBookingForm((prev) => ({ ...prev, quantity: e.target.value }))
                        }
                    ...
                    <label className="flex flex-col gap-1.5">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setBookingForm((prev) => ({ ...prev, contactNumber: e.target.value }))
                        }
                        className="w-full rounded-xl border border-gold/20 bg-[#050505] p-3 text-sm text-white outline-none placeholder:text-white/40 focus:border-gold"
                      />
                    </label>
                  ...
                  <label className="flex flex-col gap-1.5">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setBookingForm((prev) => ({ ...prev, deliveryAddress: e.target.value }))
                      }
                      className="w-full rounded-xl border border-gold/20 bg-[#050505] p-3 text-sm text-white outline-none placeholder:text-white/40 focus:border-gold"
                    />
                  </label>
                  ...
                  <label className="flex flex-col gap-1.5">
                    ...
                    <textarea
                      rows={2}
                      placeholder="Any packaging or delivery instructions..."
