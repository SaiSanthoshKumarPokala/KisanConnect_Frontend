...
import ProfileStat from "../components/fprofile/ProfileStat";
import InfoRow from "../components/fprofile/InfoRow";
...
export default function FProfile() {
  ...
  useDocumentTitle(profile?.name ? `${profile.name}'s Profile` : "Farmer Profile");

  const fetchProfileData = useCallback(async (signal) => {
    try {
      const { data } = await axios.get("/api/farmer/data", { signal });
