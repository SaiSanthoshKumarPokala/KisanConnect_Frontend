...
export default function SPProfile() {
    ...
    const { navigate, role, axios, token } = UseAppContext();
    ...
    useEffect(() => {
        async function fetchSPData() {
            ...
            // console.log(sp);
            if (sp.data.success) {
                setName(sp.data.user.name);
                setEmail(sp.data.user.email);
                setAddress(sp.data.user.address);
                setPhonenumber(sp.data.user.phonenumber);
                setDateofbirth(sp.data.user.dateofbirth);
                setPincode(sp.data.user.pincode);
                setstate(sp.data.user.state.toLowerCase());
            }
            else {
                window.alert("Wrong");
            }
